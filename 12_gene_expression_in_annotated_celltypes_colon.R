...
# 设置工作目录
workDir <- "D:\\生信分析狮\\1双疾病（结肠癌、银屑疾）\\30.研究的基因在注释细胞的表达情况"
setwd(workDir)
...
for (gene in genes_to_analyze) {
  ...
  if (n_celltypes <= 10) {  # 如果细胞类型不太多，可以放在一个图中
    ...
    p_combined <- ggplot(expr_df_filtered, aes(x = Group, y = Expression, fill = Group)) +
      ...
      facet_wrap(~Cell_Type, nrow = 2, ncol = ceiling(n_celltypes/2), scales = "free_y") +
    ...
    ggsave(combined_pdf_path, plot = p_combined, width = 14, height = 10, dpi = 300, device = "pdf")
